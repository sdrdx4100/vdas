...
| 自由分析 | **タググループ統計分析**。①タグでグループを選ぶ (タグ1つ=1グループ) → ②分析の種類をギャラリーから1つ選ぶ → ③最小限の設定、の3ステップで結果は常に1枚。分析は6種類: **集合統計 (ログ単位)** = 各ログの代表値を1標本として箱ひげ+個点・差・95%CI・効果量 / **分布 (割合%)** / **構成比** / **動作領域** (グループ別等高線) / **状態遷移・頻度** (行数・時間・距離あたり) / **区間 (イベント) 分析** = 状態列が指定値の連続区間を1イベントとして抽出し経過時間を比較 (例: shiftinprocess=1 区間の変速所要時間、イベント中の信号平均との散布図も)。正規化 (データセット均等・全行プール) とフィルタは全分析共通。1グループなら単体集計。すべて自動更新。 |
