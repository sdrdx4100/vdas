...
| 比較可視化 | 個別データセット比較に加え、タグでA/Bのデータ群を作る**比較ワークスペース**を搭載。共通ビンの1D分布、回転数×車速などの2D密度 (A / B / 差分)、変速などの状態遷移頻度を、データセット均等・全行プール・時間/距離あたりで比較可能。従来の彼我差分サマリ、CDF、箱ひげ図、特性カーブ、統計量比較も利用可能。 |
