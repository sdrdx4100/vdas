...
| GPS・地図 | **走行軌跡の地図 + 信号波形の連動表示**。GPS ログは信号ログとは別ファイル (同名の Parquet) として記録される前提で、両者を**行位置 (rowid) で 1:1 対応づけ**る (時間の流れ・行数が一致)。GPS データは座標列 (`latitude/longitude` などの緯度経度名、または `GPS_x`/`GPS_y`/`GPS_z` のような座標軸列) の有無で自動判定し、同名の信号データと**自動ペア**。座標が**緯度経度 (度)** か**ローカル座標 (メートル等)** かは実際の値域から自動判別し、前者は**実地図タイル (OpenStreetMap、トークン不要)** 上に、後者は**等尺の平面軌跡**として描く (どの2軸を使うか・緯度/経度の割り当ても値から自動、手動指定も可)。下部には信号ごとの個別軸波形。地図の軌跡は任意の信号で色分け可能。**地図・波形どちらかにカーソルを合わせると、もう一方の対応位置が連動してハイライト**する (地図→波形は縦線、波形→地図は点)。フィルタ・自動間引き対応。地図タイルの取得にはインターネット接続が必要。 |
