...
| 自由分析 | **タググループ統計分析**。①タグでグループを選ぶ (タグ1つ=1グループ) → ②分析の種類をギャラリーから1つ選ぶ → ③最小限の設定、の3ステップで結果は常に1枚。分析は9種類: **集合統計 (ログ単位)** = 各ログの代表値を1標本として箱ひげ+個点・差・95%CI・効果量 / **分布 (割合%)** / **構成比** / **動作領域** (グループ別等高線) / **状態遷移・頻度** (行数・時間・距離あたり) / **区間 (イベント) 分析** = 状態列が指定値の連続区間を1イベントとして抽出し経過時間を比較 (例: shiftinprocess=1 区間の変速所要時間、イベント中の信号平均との散布図も) / **回帰・トレンド** = Y~X の線形回帰の傾き・R² をグループ比較 / **主成分分析 (PCA)** = 多次元信号を標準化して PC1-PC2 平面へ射影しグループの分離を可視化 / **相関** = グループごとの信号間相関行列を横並び比較。集合統計には効果量 (Hedges g) に加え有意差検定 (Welch の t検定・Mann-Whitney) の p値も表示。正規化 (データセット均等・全行プール) とフィルタは全分析共通。1グループなら単体集計。すべて自動更新。 |
