...
| GPS・地図 | **走行軌跡の地図 + 信号波形の連動表示**。GPS ログ (緯度・経度) は信号ログとは別ファイル (同名の Parquet) として記録される前提で、両者を**行位置 (rowid) で 1:1 対応づけ**る (時間の流れ・行数が一致)。GPS データは緯度・経度列の有無で自動判定し、同名の信号データと**自動ペア**。上部に実地図タイル (OpenStreetMap、トークン不要) 上の走行軌跡、下部に信号ごとの個別軸波形を表示。地図の軌跡は任意の信号で色分け可能。**地図・波形どちらかにカーソルを合わせると、もう一方の対応位置が連動してハイライト**する (地図→波形は縦線、波形→地図は点)。GPS データセット・緯度経度列は手動指定も可能。フィルタ・自動間引き対応。地図タイルの取得にはインターネット接続が必要。 |
