...
| GPS・地図 | **走行軌跡の地図 + 信号波形の連動表示・2走行比較再生**。GPS ログは信号ログとは別ファイル (同名の Parquet) として記録される前提で、各ペア内を**行位置 (rowid) で 1:1 対応づけ**る。座標列と緯度経度／ローカル座標を自動判定し、実地図または等尺平面に描画する。単独走行に加え、任意の**比較走行 B**を選ぶと、行数・サンプリング周期・時間軸が異なる A/B の軌跡と同名信号波形を別色で重ねて表示する。A/Bに**独立したシークバー**があり、それぞれで同じ地点やイベントを選ぶと両者の時間差を算出する。その位置合わせを基準に、各走行の時間軸から対応点を求めながら同期再生する。Bの時間オフセットの微調整、再生／一時停止・先頭復帰・再生速度変更、地図／波形クリックによる各走行の位置選択にも対応する。フィルタ・自動間引き対応。実地図タイルの取得にはインターネット接続が必要。 |
